...
cftfile = "cft" # cleaned fault likelihood thinned
...
def goNormals():
  ...
  lof = LocalOrientFilter(16.0,1.0)
  ...
  cmap = ColorMap.JET
  plot3(u1,cmin=min(u1),cmax=max(u1),cmap=cmap,clab="u1")
  plot3(u2,cmin=min(u2),cmax=max(u2),cmap=cmap,clab="u2")
  plot3(u3,cmin=min(u3),cmax=max(u3),cmap=cmap,clab="u3")
  plot3(gx,ep,cmin=0,cmax=1,cmap=jetRamp(1.0),clab="ep")
...
def goFaultSurfer():
  ...
  fi.setIterations(0.01,200)
  fi.setSmoothings(4.0,4.0)
  fm = fi.getMappingsFromNormals(s1,s2,s3,u1,u2,u3,wp)
  fv = fm.fi
  cmap = ColorMap.JET
  plot3(fv,cmin=min(fv),cmax=max(fv),cmap=cmap,clab="fi")
...
def goNormalWeight():
  ...
  for skin in skins:
    for cell in skin:
      ...
      fl[i3][i2][i1] = 1.0#ep[i3][i2][i1]
      '''
      if i1>=n1:
        i1 = n1-1
      fl[i3  ][i2  ][i1] = ep[i3  ][i2  ][i1]
      fl[i3+1][i2  ][i1] = ep[i3+1][i2  ][i1] 
      fl[i3+2][i2  ][i1] = ep[i3+2][i2  ][i1] 
      fl[i3-1][i2  ][i1] = ep[i3-1][i2  ][i1] 
      fl[i3-2][i2  ][i1] = ep[i3-2][i2  ][i1] 
      fl[i3  ][i2+1][i1] = ep[i3  ][i2+1][i1] 
      fl[i3  ][i2+2][i1] = ep[i3  ][i2+2][i1] 
      fl[i3  ][i2-1][i1] = ep[i3  ][i2-1][i1] 
      fl[i3  ][i2-2][i1] = ep[i3  ][i2-2][i1] 
      fl[i3+1][i2+1][i1] = ep[i3+1][i2+1][i1] 
      fl[i3+2][i2+2][i1] = ep[i3+2][i2+2][i1] 
      fl[i3-1][i2-1][i1] = ep[i3-1][i2-1][i1] 
      fl[i3-2][i2-2][i1] = ep[i3-2][i2-2][i1] 
      '''
  writeImage(u1file,u1)
  writeImage(u2file,u2)
  writeImage(u3file,u3)
  writeImage(cftfile,fl)
  cmap = ColorMap.JET
  plot3(u1,cmin=min(u1),cmax=max(u1),cmap=cmap,clab="u1")
  plot3(u2,cmin=min(u2),cmax=max(u2),cmap=cmap,clab="u2")
  plot3(u3,cmin=min(u3),cmax=max(u3),cmap=cmap,clab="u3")
  plot3(fl,cmin=min(fl),cmax=max(fl),cmap=cmap,clab="wp")
